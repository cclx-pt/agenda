              ...
              {cell.events?.slice(0, 2).map(evt => {
                ...
                return (
                  <EventHoverCard key={evt.id} event={evt}>
                    <div
                      className={cn(
                        'mb-0.5 flex cursor-pointer items-center gap-[3px] overflow-hidden whitespace-nowrap rounded-sm bg-muted/60 px-1.5 py-0.5 text-[10px] font-semibold tracking-wide text-foreground hover:bg-muted max-[600px]:gap-0.5 max-[600px]:px-[3px] max-[600px]:py-px max-[600px]:text-[8px]',
                        st && '[outline:1px_dashed_currentColor] [outline-offset:-2px]',
                        evt.featured && 'cclx-featured',
                      )}
                      style={st ? { background: st.bg, color: vis.text } : undefined}
                      ...
                      <span className="h-1.5 w-1.5 flex-shrink-0 rounded-full" style={{ background: vis.dot }} />
