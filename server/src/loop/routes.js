...
loopRouter.get('/:slug', async (req, res, next) => {
  try {
    ...
    if (!cfg || !cfg.active) {
      return res.json({ active: false, church: cfg?.community || slug, weeks: cfg?.weeks ?? 4, format: cfg?.format ?? '16:9', secondsPerSlide: cfg?.secondsPerSlide ?? 15, secondsPerSlideFeatured: cfg?.secondsPerSlideFeatured ?? 30, fixedSlides: cfg?.fixedSlides ?? [], events: [] })
    }
    const now = new Date()
    const end = new Date(now)
    end.setDate(end.getDate() + cfg.weeks * 7)
    const events = uniqueLoopEvents(await eventsRepo.listForLoop({
    ...
    res.json({ active: true, church: cfg.community || cfg.name, weeks: cfg.weeks, format: cfg.format, secondsPerSlide: cfg.secondsPerSlide, secondsPerSlideFeatured: cfg.secondsPerSlideFeatured, fixedSlides: cfg.fixedSlides, events })
  } catch (err) {
    next(err)
  }
})
